...
from train_evaluate import train_one_epoch, setup_logger, CLASS_NAMES, save_report
from baselines.wang_2026.model import CoughRNet

def main():
    print("=== Wang et al. 2026 Baseline (CoughRNet CNN-SVM) ===")
    xgb_df = get_loocv_df()
    ...
    logger.info(f"Starting LOOCV | Wang 2026 (CoughRNet CNN-SVM) | {len(unique_files)} files")
    ...
    for fold_idx, (train_ds, test_ds, test_file) in enumerate(get_loocv_folds(xgb_df)):
        ...
        model = CoughRNet(num_classes=3).to(device)
